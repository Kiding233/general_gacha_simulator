...
**Goal:** 为所有统计估计添加 Bootstrap 置信区间计算，在图表和表格上可视化展示稳定性。零额外模拟成本——Bootstrap 是对已有 N 条结果做有放回抽样，纯数组操作。

**Architecture:** 新建 `BootstrapEngine` 核心类（`core/bootstrap.py`）统一处理所有 Bootstrap 计算，支持标准/BCa/m-out-of-n/参数GPD 四种重抽样方法。各分析面板通过调用 `BootstrapEngine` 的对应方法获取 CI，图表用阴影带/误差棒展示，表格用 `0.95 [0.92, 0.98]` 格式展示。

**Tech Stack:** Python 3.10+, numpy, pytest

> 日期：2026-05-18 | 修订：v2（TDD + bite-sized 重写，2026-05-24）
> 状态：未开始
> 依赖：P2（过程分析续，新增事件类型影响 `process_data` 格式）——P2 先行，P3 在 P2 完成后实现
...
- [ ] **1.3: bootstrap_probability() — 二分类概率**
...
- [ ] **1.5: bootstrap_distribution() — 连续分布分位数**
...
- [ ] **1.6: detect_heavy_tail() — Hill 估计量**
