...
| 步骤 | 内容 | 工作量 | 状态 |
|------|------|--------|------|
| 0.1 | 删除根目录 `worst_impact.py`，统一使用 `core/worst_impact.py` | 小 | ✅ 已完成（根目录已无此文件） |
...
| 步骤 | 内容 | 工作量 | 状态 |
|------|------|--------|------|
| 3.1 | `vulnerability.py` 的 `_is_success()` 替换为 `SuccessChecker` | 小 | ✅ 已完成（已使用 SuccessChecker 实例） |
| 3.2 | `streaming.py` 的 `extract_process()` 使用 `SuccessChecker` | 小 | ✅ 已完成 |
| 3.3 | `analysis_panel.py` 的 GDR 调用统一为 `SuccessChecker` | 小 | ⚠️ 不适用（analysis_panel 使用 compute_gdr_from_compact 计算分布值而非判断成功，函数选择正确） |
| 3.4 | `UNIFIED_GDR_REGISTRY` 增加 `register_gdr()` 函数，带冲突检测 | 小 | ✅ 已完成（gdr.py:505） |
...
| 2026-05-23 | v9 | 全面重新审查状态：修正 3 项误标——0.1（根目录 worst_impact.py 已不存在）、3.1（vulnerability.py 已使用 SuccessChecker）、3.4（register_gdr 已存在于 gdr.py:505）；3.3 标记为不适用（analysis_panel 使用 compute_gdr_from_compact 计算分布值，函数选择正确）；剩余未实施 3 项：2.8（worker 全局变量）、4.4（策略参数动态控件）、4.6（GUI 面板 self.window()，实际已通过 set_config_panel 解决）。`modification_plan.md` 所有条目已完成并移至 docs/。 |
