...
**2026-05-28 更新**：P24（EVT 尾部拟合）已完成，`evt_tail.py` 提供 `fit_gpd_lower()` / `fit_gpd_upper()` / `evt_var_right()` 等统一接口。P25（EVT 改进）审查发现 B2.8（EVT 拟合路径统一）和 B2.9（TIB 替代百分位法）两个新需求。详见 `docs/EVT改进——离散型与退化分布处理.md` §八。
