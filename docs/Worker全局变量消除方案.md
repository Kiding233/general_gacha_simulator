...
> 编写日期：2026-05-22 | 修订：v2（checkbox bite-sized 格式 + 测试策略，2026-05-24）
...
- [ ] **2.8.1: 将 11 个 `_wk_*` 全局变量合并为 1 个 `_wk_env`**
...
- [ ] **2.8.2: 重构 `_wk_init()` 接受 `SimulationEnv` 单一参数**
...
- [ ] **2.8.3: 重构 `_wk_run_single()` 从 `_wk_env` 读取字段**
...
- [ ] **2.8.4: 重构 `run_batch_parallel()` 接受 `env: SimulationEnv` 参数**
...
- [ ] **2.8.6: 单进程路径不再调用 `_wk_init()`，直接使用 `env`**

- [ ] **2.8.7: `SimulationEnv` 添加缺失字段**
...
并在 `SimulationEnvBuilder.from_config_store()` 中填充。

- [ ] **2.8.8: 运行全部测试确认无回归**

```bash
pytest tests/ -v
# 预期: 全部 PASS（worker 机制变更不影响核心逻辑）
```

- [ ] **2.8.9: 删除旧 `_wk_*` 全局变量和 `_wk_init()` 函数（只保留 1 个 `_wk_env`）→ 提交**

```bash
git add gacha_simulator/service/batch_simulator.py \
        gacha_simulator/gui/gacha_panel.py \
        gacha_simulator/gui/strategy_panel.py \
        gacha_simulator/gui/resource_search_panel.py \
        gacha_simulator/gui/strategy_comparison_panel.py \
        gacha_simulator/core/worst_impact.py \
        gacha_simulator/core/retreat_search.py
git commit -m "refactor: Worker 全局变量消除——11个_wk_*合并为1个_wk_env SimulationEnv"
```
